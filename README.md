...
- **[工作流程](docs/WORKFLOW.md)** - 详细的调用关系与数据流
- **[开发指南](docs/DEVELOPER_GUIDE.md)** - 模块化开发最佳实践

### 专题指南 / Specialized Topics
...
# 重试策略
MAX_RETRIES = 10               # 技术错误：10次 + 指数退避
PDF_BATCH_RETRIES = 5          # 质量错误：5次快速重试
```

### 图片处理参数

```python
# 图片清洗标准（宽松模式，v2.11.2优化）
...
│   ├── outline.jsonl
│   ├── translation.jsonl
│   ├── html_generation.jsonl
│   └── vision.jsonl
├── components/         # 组件运行日志
│   ├── pdf_extractor_*.log
│   └── image_cleaner_*.log
└── failed_files.jsonl  # 失败文件记录（v2.12.2+）
```
...
- **整体成功率**：99.5%+
- **超时率**：<2%（从20%优化至2%）
- **图片位置获取成功率**：90%+（v2.10.1修复bbox bug）
...
- 🖼️ **[图片处理](docs/IMAGE_PROCESSING.md)** - 图片处理完整流程
- 🔧 **[Vision API](docs/VISION_API.md)** - Vision API集成指南
- ⚙️ **[配置说明](docs/CONFIGURATION.md)** - 配置参数详解
...
- 翻译失败 → 检查API配额和术语库格式
- 输出文件不完整 → 检查磁盘空间和缓存文件
- 断点续传问题 → 使用 `recover_progress_from_json.py` 恢复进度
